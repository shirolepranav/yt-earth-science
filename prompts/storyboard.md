You are the editor of a dark, investigative finance documentary on YouTube, cut in the style of ColdFusion and Moon: real footage that shows exactly what is being said, cut when the narration moves on, and the scary claims backed by on-screen receipts. Plan the shots for one stretch of narration.
...
Beat kinds:
  * "stock" - ALL footage. Real human-shot footage from Pexels or Pixabay. Give "queries": three different short searches (two to five concrete, visual words each) a stock library would really have, from different angles on the same moment - e.g. ["eviction notice on apartment door", "person reading letter at kitchen table", "apartment hallway at night"]. Give "intent": one sentence saying exactly what must be visible for the shot to fit the narration. Short clips are fine; relevance is what matters. Never ask for charts, screens with numbers, collages or illustrations. Also give "image_prompt" (a concrete, photographic description of one frame that shows the same thing: subject, setting, framing, light; ordinary, clean, recognisable places - no blood, stains, decay, squalor or horror; no charts, numbers, calculators or screens with data; no faces as the subject, no readable text) and "motion_prompt" (one slow camera move) - used only if no library has a usable clip.
  * "chart" - one of the charts above, by "chart_index", exactly when the narration gives its numbers. OR, when this stretch of narration compares two or more figures that have no chart above, a new chart built only from those spoken figures: give "data": {{"chart_kind": "bar" | "line" | "comparison", "title": "short title", "labels": ["...", "..."], "values": [21, 2], "unit": "%" or "$" or a short word, "source": "short name"}} - 2 to 8 values, numbers exactly as spoken, at most {extra_charts_left} more of these in the whole video.
  * "number" - one huge key figure on screen when the narration lands it. Give "value" (as it should appear, e.g. "$40,000" or "52"), "label" (at most 6 words), "source" (short name). About {number_target} in this stretch, at most {numbers_left} left for the whole video - the figure must be in the narration.
  * "evidence" - the receipts: a source page with one sentence highlighted. Give "source_index" and "quote": one sentence copied EXACTLY, character for character, from that source above. These and the charts are what make the film look real: this stretch needs about {evidence_target} of them, placed wherever the narration cites a source, quotes a finding, or makes an alarming claim. Pick sentences of roughly 15 to 25 words - long enough to carry the claim, short enough to read in six seconds. Never invent or trim a quote; if no sentence fits word for word, use a different kind of beat.
  * "card" - a ColdFusion-style text card, about {card_target} in this stretch at section openings, at most {cards_left} left. "variant": "question" (at a section opening, e.g. "WHY WOULD A BANK DO THIS?") or "fact" (a key figure, e.g. "2025 FEES: $18 BILLION"). "text": at most 6 words, ALL CAPS. Only figures that are in the narration. Its "image_prompt" is the photographic SCENE behind the card (a place or object from the look bible) - never a description of the card, its text or typography.
...
{{
  "beats": [
    ...
    {{"sentence": 0, "from_words": "while the landlord", "kind": "stock", "queries": ["...", "...", "..."], "intent": "...", "image_prompt": "...", "motion_prompt": "..."}},

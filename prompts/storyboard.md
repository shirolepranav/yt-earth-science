...
Beat kinds:
  ...
  * "number" - one huge key figure on screen when the narration lands it. Give "value" (as it should appear, e.g. "252 million years" or "9.1"), "label" (at most 6 words), "source" (short name). About {number_target} in this stretch, at most {numbers_left} left for the whole video - the figure must be in the narration and come from a named source. Never a number from an illustrative example or scene (e.g. the imagined spot where you stand on the shore) - those are not facts.
  ...
  * "card" - a documentary text card, about {card_target} in this stretch at section openings, at most {cards_left} left. "variant": "question" (at a section opening, e.g. "WHERE DID THE OCEAN GO?") or "fact" (a key figure, e.g. "MAGNITUDE 9.1"). "text": at most 6 words, ALL CAPS. Only figures that are in the narration. A card states a sourced fact or a conclusion the video draws - never a detail from an illustrative example or scene. Its "image_prompt" is the photographic SCENE behind the card (a place or object from the look bible) - never a description of the card, its text or typography.

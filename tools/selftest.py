...
the logic that decides what reaches the screen and what it costs:
  ...
  6. subtitle cues: two lines of 42 characters, no overlaps, no word lost;
     and that a stage whose output has vanished is not treated as done
...
def check_resume_honesty() -> None:
    ...
    This is the CI-versus-laptop trap: `stages_done` is committed to the repo,
    but `audio/` and `assets/` are gitignored because they're far too big for
    git. So a resumed build is handed a state file claiming the audio stage
    finished, on a machine where the WAV has never existed. It skipped to the
    render and died on a missing file after a three-hour wait.
    """
...
def main() -> None:
    ...
    print("6/6 subtitle cues and resume honesty ok")
    print("\nSELF-TEST PASSED")
